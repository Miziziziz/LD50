...
func _process(delta):
	...
	if time_alive > time_to_activate_wind:
		activate_wind()
		time_to_activate_wind += rand_range(min_wind_inc, max_wind_inc)
	if time_alive > time_to_throw_rocks:
		spawn_rock()
		var rt = rand_range(min_rock_throw_inc, max_rock_throw_inc)
...
func spawn_rock():
	...
	rock_inst.global_position.y = rand_range(rock_top_height, rock_bot_height)
